...
class BuzzpulseChatSupabaseUser extends BaseAuthUser {
  BuzzpulseChatSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> buzzpulseChatSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = BuzzpulseChatSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
